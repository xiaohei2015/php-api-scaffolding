...
use common\components\tools\sendcloud\lib\util\HttpClient;
use common\components\tools\sendcloud\lib\util\Mail;
use common\components\tools\sendcloud\lib\Config;
...
class SendCloud {
	...
	public function sendCommon(Mail $mail) {
        ...
        $res = Json::decode($resonse->body(),true);
        if($res["statusCode"]=="200"){
            return true;
        }else{
            \Yii::warning('Send mail failed, error details：'.$resonse->body());
            return false;
        }
	}
